...
const SongPage = ({ usuario }) => {
  ...
  }); // 'album' | 'coleccion' | 'lista'
  const [contextId, setContextId] = useState(() => {
    const p = new URLSearchParams(location.search);
    return p.get('contextId') || null;
  });
  const [contextList, setContextList] = useState([]); // ordered list of ids or song objects
...
useEffect(() => {
  ...
  // Helper: obtener info mínima de una cancion
  const fetchSongMinimal = async (songId) => {
    try {
      const res = await axios.get(`${API_URL}/canciones/${songId}`);
      return res.data;
    } catch (error) {
      console.error('Error fetching song minimal:', error);
      return null;
    }
  };

  useEffect(() => {
    // Fetch album songs and context list
    const fetchAll = async () => {
      try {
        // 1. Obtener canciones del álbum
        if (album && album.id_album) {
          const albumRes = await axios.get(`${API_URL}/relaciones/albumes/${album.id_album}/canciones`);
          const list = Array.isArray(albumRes.data.canciones) ? albumRes.data.canciones : [];
          setAlbumSongs(list);
        }

        // 2. Manejar contexto según tipo
        if (contextType === 'album' && contextId) {
          // Si el contexto es album, usa las canciones del álbum como contextList
          if (album && album.id_album && String(album.id_album) === String(contextId)) {
            setContextName(album.titulo || album.nombre || null);
          } else {
            try {
              const a = await axios.get(`${API_URL}/albumes/${contextId}`);
              setContextName(a.data.titulo || a.data.nombre || null);
            } catch (e) {
              setContextName(null);
            }
          }
          ...
          try {
            const res = await axios.get(`${API_URL}/listas-personalizadas/elementos/${contextId}`);
            const items = Array.isArray(res.data) ? res.data : [];
            ...
            try {
              const c = await axios.get(`${API_URL}/listas-personalizadas/detalle/${contextId}`);
              setContextName(c.data.nombre_lista || null);
            } catch (e) {
              setContextName(null);
            }
          } catch (e) {
        ...
        } else if (album && album.id_album) {
          // Sin contexto explícito, usa canciones del álbum como fallback
          setContextList([]);
        }
      } catch (error) {
        console.error('Error fetching album or context:', error);
      }
    };
  ...
  const prevSong = useMemo(() => {
    ...
    // Si hay contexto y contextList, usa contextList
    if (contextType && contextList && contextList.length > 0) {
      currentIdx = contextList.findIndex(c => String(c) === String(id));
      if (currentIdx > 0) {
        const prevId = contextList[currentIdx - 1];
        // Si es album, busca en albumSongs; si no, solo retorna id
        if (contextType === 'album' && albumSongs && albumSongs.length > 0) {
          return albumSongs.find(s => String(s.id_cancion) === String(prevId)) || { id_cancion: prevId };
        }
        return { id_cancion: prevId };
      }
      return null;
    }

    // Fallback a albumSongs
    if (albumSongs && albumSongs.length > 0) {
      currentIdx = albumSongs.findIndex(c => String(c.id_cancion) === String(id));
      return currentIdx > 0 ? albumSongs[currentIdx - 1] : null;
    }

    return null;
  }, [id, contextType, contextList, albumSongs]);
  ...
  const nextSong = useMemo(() => {
    ...
    if (contextType && contextList && contextList.length > 0) {
      ...
      if (currentIdx >= 0 && currentIdx < contextList.length - 1) {
        const nextId = contextList[currentIdx + 1];
        // Si es album, busca en albumSongs; si no, solo retorna id
        if (contextType === 'album' && albumSongs && albumSongs.length > 0) {
          return albumSongs.find(s => String(s.id_cancion) === String(nextId)) || { id_cancion: nextId };
        }
        return { id_cancion: nextId };
      }
      return null;
    }
    ...
    if (albumSongs && albumSongs.length > 0) {
      currentIdx = albumSongs.findIndex(c => String(c.id_cancion) === String(id));
      return currentIdx >= 0 && currentIdx < albumSongs.length - 1 ? albumSongs[currentIdx + 1] : null;
    }
  ...
  useEffect(() => {
    ...
    const fetchInfos = async () => {
      if (prevSong) {
        if (!prevSong.titulo) {
          const data = await fetchSongMinimal(prevSong.id_cancion);
          if (mounted) setPrevInfo(data);
        } else if (mounted) setPrevInfo(prevSong);
      } else if (mounted) setPrevInfo(null);

      if (nextSong) {
        if (!nextSong.titulo) {
          const data = await fetchSongMinimal(nextSong.id_cancion);
          if (mounted) setNextInfo(data);
        } else if (mounted) setNextInfo(nextSong);
      } else if (mounted) setNextInfo(null);
    };
    fetchInfos();
    return () => { mounted = false; };
  }, [prevSong?.id_cancion, nextSong?.id_cancion]);
          ...
          {contextType && (
            <div className="text-center mb-2">
              <span className="inline-block bg-gray-100 text-gray-800 px-2 py-1 rounded text-sm">
                Navegando por: {contextName ? contextName : `${contextType}${contextId ? ' ' + contextId : ''}`}
              </span>
            </div>
          )}
  ...
  <div style={{ flex: 1, textAlign: "left" }}>
    {prevSong ? (
      (() => {
        const prevId = prevSong.id_cancion || prevSong.id;
        const title = prevSong.titulo || prevInfo?.titulo || 'Anterior';
        const query = contextType && contextId ? `?context=${contextType}&contextId=${contextId}` : '';
        return (
          <Link to={`/song/${prevId}${query}`} className="text-blue-600 flex items-center">
  ...
  <div style={{ flex: 1, textAlign: "right" }}>
    {nextSong ? (
      (() => {
        const nextId = nextSong.id_cancion || nextSong.id;
        const title = nextSong.titulo || nextInfo?.titulo || 'Siguiente';
        const query = contextType && contextId ? `?context=${contextType}&contextId=${contextId}` : '';
        return (
          <Link to={`/song/${nextId}${query}`} className="text-blue-600 flex items-center justify-end">
