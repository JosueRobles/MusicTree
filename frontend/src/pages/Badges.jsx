...
const Badges = ({ usuario }) => {
  ...
  const [progresoInsignias, setProgresoInsignias] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchInsigniasUsuario = async () => {
      if (usuario) {
        try {
          // Obtener insignias desbloqueadas (tabla insignias_usuario)
    ...
    const fetchProgresoInsignias = async () => {
      if (usuario) {
        try {
          // Obtener progreso dinámico calculado desde la vista
          const response = await axios.get(`${API_URL}/insignias/progreso-dinamico/${usuario.id_usuario}`);
          setProgresoInsignias(Array.isArray(response.data) ? response.data : []);
        } catch (error) {
          console.error("Error al obtener progreso de insignias:", error);
          setProgresoInsignias([]);
        }
      }
    };
  ...
  // Busca el progreso de la insignia (desde la vista, dinámico)
  const obtenerProgresoInsignia = (insigniaId) => {
    return progresoInsignias.find((prog) =>
      String(prog.id_insignia) === String(insigniaId)
    );
  };

  const totalInsignias = insignias.length;
  const obtenidas = progresoInsignias.filter(prog =>
    prog.progreso !== undefined &&
    !isNaN(prog.progreso) &&
    Number(prog.progreso) >= 100
  ).length;
