...
// Obtener progreso dinámico de insignias desde la vista (sin efectos secundarios)
const obtenerProgresoInsignias = async (req, res) => {
  const { userId } = req.params;
  if (!userId || isNaN(Number(userId))) {
    return res.status(400).json({ error: 'userId requerido y debe ser numérico.' });
  }
  try {
    // Consulta la vista de progreso
    const { data: progreso, error } = await supabase
      .from('vista_progreso_insignias')
      .select('*')
      .eq('id_usuario', userId);
    ...
    res.status(500).json({ error: 'Error al obtener progreso de insignias.' });
  }
};
